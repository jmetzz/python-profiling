...
def generate_data(factor):
    ...
    data_df = pd.DataFrame(
        data={"item_id": item_ids, "price": prices, "price_constant": price_constants, "units": units,}
    )
    return data_df
...
def form_output(data_df):
    ...
    for i in range(len(data_df)):
        item_output = {
            ...
            "priceVars": {"price": data_df.loc[i, "price"], "priceConstant": data_df.loc[i, "price_constant"],},
